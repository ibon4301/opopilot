...
**Búsqueda con RLS, no a pesar de él.** La RPC `match_document_chunks` es `SECURITY INVOKER`: corre como el usuario autenticado, así que las policies de RLS aplican dentro de la función, con filtro explícito por `auth.uid()` como defensa en profundidad, y `EXECUTE` revocado para `anon`. Ordena por distancia coseno (`<=>`, índice HNSW) y devuelve chunk, documento, página y similitud. La Server Action de búsqueda comprueba que hay chunks indexados **antes** de gastar la llamada a Gemini que embebe la consulta.
...
**Un único modelo por política de costes.** Toda la generación (tests y flashcards) usa exclusivamente `gemini-3.1-flash-lite` con `temperature` baja y sin presupuesto de "thinking". No hay cadenas de fallback a modelos más caros: si el modelo no está disponible (404) se devuelve un error controlado, nunca se cambia de modelo automáticamente. Los errores comunes de la API (key inválida, rate limit, saturación) se traducen en un único sitio (`services/gemini/errors.ts`) compartido por todos los proveedores.
...
## Hacer y corregir tests (Fase 8)

**La corrección vive en el servidor.** `getTestAction` selecciona explícitamente las columnas que viajan al cliente (`TakeableQuestion`): la opción correcta y la explicación **nunca** salen del servidor antes de corregir. `submitTestAttempt` (`services/test-attempts/`) exige el test completo, califica contra la base de datos y persiste el intento en `test_attempts` + `test_attempt_answers`; su respuesta es el primer momento en que correcta y explicación llegan al cliente. Cada corrección es un intento independiente — el historial queda listo para estadísticas futuras.

## Flashcards con IA (Fase 9)

**Mismo patrón que tests: contrato + proveedor + orquestación.** `services/flashcard-generation/` replica la estructura de `test-generation/` (provider, gemini-provider, prompts, schemas Zod → `responseJsonSchema`, validators con número exacto/duplicados, un solo reintento, `insufficientContext` sin reintento) y reutiliza `services/document-context/` para el contexto y el cliente Gemini compartido. El tipo de tarjeta (pregunta-respuesta, concepto-definición o mixto) solo condiciona el prompt, no el esquema. `sourcePage` se sanea en el servicio: cualquier página citada por el modelo que no esté en el contexto real se descarta a `null`.

**El mazo es el agregado.** `flashcard_decks` agrupa las tarjetas (la tabla `flashcards` de la Fase 3 ganó `deck_id`, `hint`, `difficulty`, `source_page` y `order_index`); borrar el mazo arrastra tarjetas, contexto (`flashcard_deck_context_chunks`) y reviews en cascada. La propiedad de una review la garantiza el esquema: la FK compuesta `(flashcard_id, user_id) → flashcards` solo existe si la tarjeta es del usuario, así que valorar una tarjeta ajena viola la FK sin necesidad de comprobación adicional.

**Estudio simple, base para repetición espaciada.** La sesión de estudio es estado local (anverso → revelar → valorar) y cada valoración inserta en `flashcard_reviews` (`again | hard | good | easy`). A diferencia de los tests, el reverso sí viaja al cliente: las flashcards son autoevaluación del propio usuario, así que ocultarlo en el servidor añadiría una petición por tarjeta sin proteger nada — la UI no lo renderiza hasta revelar. Todavía no hay SM-2/FSRS: los campos de `flashcards` (ease_factor, due_at…) siguen intactos y el historial de reviews es la materia prima con la que la fase de plan de estudio calculará qué tarjetas tocan cada día.
